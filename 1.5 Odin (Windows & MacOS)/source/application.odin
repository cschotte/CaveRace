...
run_application :: proc(options: Launch_Options) -> bool {
	...
	if verbose do rl.SetTraceLogLevel(.ALL)

	app: Application
	app.verbose_logging = verbose
	log_step(verbose, "Resolving resource root...")
	...
	log_step(verbose, "Resource root: %s", app.resource_root)
	defer delete(app.resource_root)

	settings := default_settings()
	if path, path_ok := settings_path(); path_ok {
		app.settings_path = path
		log_step(verbose, "Settings path: %s", app.settings_path)
		if loaded, loaded_ok := load_settings_from_path(path); loaded_ok {
			settings = loaded
			log_step(verbose, "Loaded settings from disk.")
		} else {
			log_step(verbose, "No usable settings file found; using defaults.")
		}
	} else {
		log_step(verbose, "Could not resolve a settings path; using defaults for this session.")
	}
	...
	log_step(verbose, "Render texture ready.")
	defer rl.UnloadRenderTexture(app.canvas)

	log_step(verbose, "Calling InitAudioDevice()...")
	rl.InitAudioDevice()
	app.audio_ready = rl.IsAudioDeviceReady()
	log_step(verbose, "Audio device ready: %t", app.audio_ready)
	defer {
	...
	log_step(verbose, "Loading game assets...")
	assets_loaded := load_assets(&app.assets, app.resource_root, app.audio_ready, verbose)
	defer unload_assets(&app.assets)
	if !assets_loaded {
		fmt.eprintln("Failed to load one or more game assets.")
		return false
	}
	log_step(verbose, "All game assets loaded and validated.")

	// Disable the default exit key (ESC) to allow custom handling
	rl.SetExitKey(.KEY_NULL)
	apply_display_settings(&app)
	log_step(verbose, "Display settings applied.")
	...
	log_step(verbose, "Entering main loop.")
...
// save_gameplay_screenshot exports the current canvas as a timestamped PNG
// next to the settings file, reviving the original 1.2 cheat-mode "1" key.
// This lands under the same per-user config directory as settings.json
// (e.g. ~/Library/Application Support/CaveRace/Screenshots on macOS) rather
// than the real Desktop, since a sandboxed App Store build has no
// entitlement for silent Desktop writes; falling back to the working
// directory keeps the cheat usable even if that lookup fails.
save_gameplay_screenshot :: proc(canvas: rl.RenderTexture2D) {
	...
	path := name
	if config_root, config_error := os.user_config_dir(context.temp_allocator); config_error == nil {
		if directory, directory_error := filepath.join(
			{config_root, SETTINGS_DIRECTORY_NAME, "Screenshots"},
			context.temp_allocator,
		); directory_error == nil {
			directory_ready := os.exists(directory) || os.make_directory_all(directory) == nil
			if directory_ready {
				if joined, join_error := filepath.join({directory, name}, context.temp_allocator); join_error == nil {
					path = joined
				}
			}
		}
...
process_game_requests :: proc(app: ^Application, result: ^Game_Update_Result) {
	if result.load_level_requested {
		log_step(app.verbose_logging, "Loading level %d...", app.game.gameplay.level_index)
		load_gameplay_level(&app.game.gameplay, app.resource_root)
		log_step(
			app.verbose_logging,
			"Level load finished, state: %v",
			app.game.gameplay.state,
		)
	}
	if result.settings_changed {
		log_step(app.verbose_logging, "Saving settings to %s", app.settings_path)
	...
	if result.display_changed {
		log_step(app.verbose_logging, "Applying display settings change...")
		apply_display_settings(app)
	}
	if result.quit_requested {
		log_step(app.verbose_logging, "Quit requested.")
