...
// partial resources and leaves the bundle empty. With verbose enabled, each
// group is logged before/after loading so a crash partway through points
// straight at the offending file.
load_assets :: proc(assets: ^Assets, resource_root: string, load_audio := true, verbose := false) -> bool {
	assets^ = {}
	log_step(verbose, "Loading screen textures...")
	...
	for relative_path, image_index in FRONT_END_TEXTURE_PATHS {
		log_step(verbose, "Loading front-end texture: %s", relative_path)
		assets.screens.front_end[image_index] =
			load_resource_texture(resource_root, relative_path)
	}
	log_step(verbose, "Screen textures loaded.")

	if load_audio {
		log_step(verbose, "Loading sound effects...")
		for relative_path, sound_index in BOMB_SOUND_PATHS {
			log_step(verbose, "Loading sound: %s", relative_path)
		...
		log_step(verbose, "Loading music streams...")
		for relative_path, cue in MUSIC_PATHS {
			log_step(verbose, "Loading music: %s", relative_path)
			assets.music[cue] = load_resource_music(resource_root, relative_path)
		}
		log_step(verbose, "Music streams loaded.")
	}

	log_step(verbose, "Loading sprite sheets...")
	...
	log_step(verbose, "Terrain tiles loaded.")

	log_step(verbose, "Validating loaded assets...")
	if !assets_are_valid(assets, load_audio) {
		fmt.eprintln("Asset validation failed; see -log output above for the last file loaded.")
		unload_assets(assets)
		return false
	}
	log_step(verbose, "Asset validation passed.")
	return true
}
